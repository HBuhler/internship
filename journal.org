...
*** work planned [1/]
- [ ] test idle event ; should be correctly gathered, see for futher
  use ; double check potential bug with use of Instant
- [ ] continue Scala Handler, may be useful to EQ, and will be for
  Watwin (same error,...)
- [ ] find group of student/exo to have more pertinent test
- [ ] add article from Chris Piech to biblio
- [ ] add Coron somewhere in journal
** week 8 june
*** work planned [1/]
- [ ] see previous TODO list, a lot of technical work
- [ ] integrate to biblio (from summary) : chries Piech biblio,
  probably a little on neural network,
- [ ] get back to compare metrcis ; try to have a functionnal proto ?
